...
export default defineFakeRoute([
    // 用户管理
    {
        ...
        response: ({ body }) => {
            ...
            if (body.phone)
                list = list.filter(item => item.phone === body.phone);
            if (body.deptId)
    ...
    // 角色管理
    {
        ...
        response: ({ body }) => {
            ...
            list = list.filter(item => String(item.status).includes(String(body?.status)));
            if (body.code)
                list = list.filter(item => item.code === body.code);
            return {
                success: true,
                data: {
                    list,

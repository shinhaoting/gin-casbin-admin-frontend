...
export default defineComponent({
    ...
    setup(props, { emit }) {
        ...
        const echoView = item => {
            ...
            addClass(instance.refs["hsdiv" + props.HsKey + item[0]], activeClass, stayClass);
            addClass(instance.refs["hstd" + props.HsKey + item[0]], bothLeftSides);
            addClass(instance.refs["hsdiv" + props.HsKey + item[1]], activeClass, stayClass);
          ...
          <tbody>
            <tr>
              {props.max.map((item, key) => {
                return (<td data-index={props.HsKey} ref={`hstd${props.HsKey}${key}`} class={`hs-select__item${key}`} onMousemove={() => setCurrentValue(key)} onMouseleave={() => resetCurrentValue(key)} onClick={() => selectValue(key, item)} style={{

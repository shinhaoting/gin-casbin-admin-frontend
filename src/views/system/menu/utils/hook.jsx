...
export function useMenu() {
    ...
    const columns = [
        ...
        },
        {
            ...
            prop: "menuType",
            width: 100,
            cellRenderer: ({ row, props }) => (<el-tag size={props.size} type={getMenuType(row.menuType)} effect="plain">
    ...
    function openDialog(title = "新增", row) {
        addDialog({
            ...
            beforeSure: (done, { options }) => {
                ...
                function chores() {
                    message(`您${title}了菜单名称为${transformI18n(formData.title)}的这条数据`, {
                        type: "success"
                    });
                    done(); // 关闭弹框
                    onSearch(); // 刷新表格数据
                ...
                FormRef.validate(valid => {
                    if (valid) {
                        ...
                        if (title === "新增") {
                            ...
                            updateMenu(submitData).then((res) => {
                                if (res.code === 200) {
                                    chores();
                                }
                            });
